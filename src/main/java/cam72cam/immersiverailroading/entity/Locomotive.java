...
public abstract class Locomotive extends FreightTank {
    ...
    @Override
    public boolean openGui(final Player player) {
        return false;
    }

    @Override
    public void handleKeyPress(final Player source, KeyTypes key,
        ...
        switch (key) {
            ...
            case DEAD_MANS_SWITCH:
                ...
                break;
            case SIFA:
                setControlGroup("sifa", 1);
                setControlGroup("sifa", 0);
                break;
            case PZBB:
                if (getControlGroup("pzbb") == 0) {
                    setControlGroup("pzbb", 1);
                } else {
                    setControlGroup("pzbb", 0);
                }
                break;
            case PZBF:
                setControlGroup("pzbf", 1);
                setControlGroup("pzbf", 0);
                break;
            case PZBW:
                setControlGroup("pzbw", 1);
                setControlGroup("pzbw", 0);
                break;
            default:
    ...
    public void onDrag(final Control<?> component, final double newValue) {
        ...
        // System.out.println("DRAG " + component + ": "+
        // getControlPosition(component));
    ...
    public ClickResult onClick(final Player player, final Player.Hand hand) {
        if (player.getHeldItem(hand).is(IRItems.ITEM_RADIO_CONTROL_CARD)
            ...
            if (this.gauge.isModel() || this.getDefinition().getRadioCapability()
                    || !Config.ConfigBalance.RadioEquipmentRequired) {
                ItemRadioCtrlCard.Data data = new ItemRadioCtrlCard.Data(player.getHeldItem(hand));
                if (player.isCrouching()) {
                    player.sendMessage(data.linked == null ? ChatText.RADIO_NOLINK.getMessage()
                            : ChatText.RADIO_UNLINK.getMessage());
                    data.linked = null;
                } else {
                    player.sendMessage(data.linked == null ? ChatText.RADIO_LINK.getMessage()
                            : ChatText.RADIO_RELINK.getMessage());
                    data.linked = this.getUUID();
                }
                data.write();
            } else {
                player.sendMessage(
                        ChatText.RADIO_CANT_LINK.getMessage(this.getDefinition().name()));
                ;
            }
            return ClickResult.ACCEPTED;
        }
    ...
    public boolean canFitPassenger(final Entity passenger) {
        if (passenger instanceof Player
                && !((Player) passenger).hasPermission(Permissions.BOARD_LOCOMOTIVE))
            return false;
    ...
    public void onTick() {
        ...
        if (getWorld().isServer) {
            ...
            for (Control<?> control : getDefinition().getModel().getControls()) {
                ...
                if (!getDefinition().isLinearBrakeControl()
                        && control.part.type == ModelComponentType.TRAIN_BRAKE_X) {
                    setTrainBrake(Math.max(0, Math.min(1,
                            getTrainBrake() + (getControlPosition(control) - 0.5f) / 8)));
                }
            }
        ...
        if (getWorld().isServer) {
            if (getDefinition().isCog() && getTickCount() % 20 == 0) {
                ...
                if (state != null) {
                    ITrack found = MovementTrack.findTrack(getWorld(), state.couplerPositionFront,
                            state.yaw, gauge.value());
    ...
    protected final double getStaticTractiveEffort(final Speed speed) {
        return (Config.ConfigBalance.FuelRequired ? this.getWeight() : this.getMaxWeight()) // KG
                ...
                * slipCoefficient(speed) * (4 / getDefinition().factorOfAdhesion()) // Physics are
                                                                                    ...
                                                                                    // adhesion
                                                                                    // factor of 4
                * Config.ConfigBalance.tractionMultiplier; // 583000
    }
    ...
    public double getTractiveEffortNewtons(final Speed speed) {
        ...
        double appliedTractiveEffort = getAppliedTractiveEffort(speed); // 5.400

        if (!cogging && Math.abs(appliedTractiveEffort) > 0) {
            double staticTractiveEffort = getStaticTractiveEffort(speed); // 583.000
            ...
            if (Math.abs(appliedTractiveEffort) > staticTractiveEffort) {
                ...
                double tractiveEffortNewtons = staticTractiveEffort / STEEL.staticFriction(STEEL)
                        * STEEL.kineticFriction(STEEL); // 350.000
                ...
                return Math.copySign(staticTractiveEffort, appliedTractiveEffort);
            }
        }

        return appliedTractiveEffort; // 5.400
    }
    ...
    @Override
    public void setThrottleLua(final float val) {
        setThrottle(val);
//		ModCore.info("Original value: " + val);
//		ModCore.info("Throttle_X: " + ModelComponentType.THROTTLE_X);
    }

    public void setThrottle(final float newThrottle) {
...
//		ModCore.info("Set Throttle to: " + newThrottle);
    ...
    @Override
    public void setReverserLua(final float val) {
        setReverser(val);
    }

    public void setReverser(final float newReverser) {
    ...
    public void setHorn(final int val, final UUID uuid) {
        if (uuid == null) {
            // Legacy API
            hornPull = 1;
        }
    ...
    public void setHorn(final int time, final float value) {
        hornTime = time;
        hornPull = value;
    }
    ...
    @Override
    public void setBrakeLua(final float val) {
        setTrainBrake(val);
    }
    ...
    @Override
    public void setIndependentBrakeLua(final float val) {
        setIndependentBrake(val);
    }

    @Override
    public void setIndependentBrake(final float newIndependentBrake) {
    ...
    private void setRealIndependentBrake(final float newIndependentBrake) {
    ...
    public void setBell(final int newBell) {
