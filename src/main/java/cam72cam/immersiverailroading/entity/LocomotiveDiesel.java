...
public class LocomotiveDiesel extends Locomotive {
    ...
    private void setEngineTemperature(final float temp) {
    ...
    public void setEngineOverheated(final boolean value) {
    ...
    public double getAppliedTractiveEffort(final Speed speed) {
        if (isRunning() && (getEngineTemperature() > 75 || !Config.isFuelRequired(gauge))) {
            ...
            double maxPowerAtSpeed =
                    maxPower_W * efficiency / Math.max(1, Math.abs(speed.metersPerSecond()));
    ...
    public void onTick() {
        ...
        float coolDownSpeed =
                heatUpSpeed * Math.copySign((float) Math.pow(ambientDelta / 130, 2), ambientDelta);
        ...
        if (this.getLiquidAmount() > 0 && isRunning()) {
            ...
            burnTime *= 10; // This is a workaround for the 10x tank size bug that existed for a
                            // long time and was tuned to
