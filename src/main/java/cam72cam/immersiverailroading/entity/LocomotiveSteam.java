...
public class LocomotiveSteam extends Locomotive {
    ...
    private void setBoilerTemperature(final float temp) {
    ...
    private void setBoilerPressure(final float temp) {
        boilerPressure = temp;
    }
    ...
    public double getAppliedTractiveEffort(final Speed speed) {
        ...
        double traction_N = Math.max(this.getDefinition().getStartingTractionNewtons(gauge), // 214.000
                this.getDefinition().getHorsePower(gauge) * 375
                        / Math.max(Math.abs(speed.imperial()), 1.0) // 675.000
        );
        if (Config.isFuelRequired(gauge)) {
            traction_N =
                    traction_N / this.getDefinition().getMaxPSI(gauge) * this.getBoilerPressure(); // 675.000
                                                                                                   // :
                                                                                                   // 232
                                                                                                   // =
                                                                                                   // 2.900
        ...
        double multiplier = Math.copySign(
                Math.abs(Math.pow(getThrottle() * Math.min(Math.abs(reverser), maxReverser), 3)),
                reverser);

        return traction_N * multiplier; // 675.000 * 0,008 = 5400
    }
    ...
    public void onTick() {
            ...
            if (this.getTickCount() % 20 == 0 && this.getDefinition().tender_auto_feed) {
                ...
                for (int slot = 2; slot < this.cargoItems.getSlotCount(); slot++) {
                    if (BurnUtil.getBurnTime(this.cargoItems.get(slot)) != 0) {
                        for (int tenderSlot = 0; tenderSlot < tender.cargoItems
                                .getSlotCount(); tenderSlot++) {
                            if (this.cargoItems.get(slot).is(tender.cargoItems.get(tenderSlot))) {
                                if (this.cargoItems.get(slot).getLimit() > this.cargoItems.get(slot)
                                        .getCount()) {
                                    ItemStack extracted =
                                            tender.cargoItems.extract(tenderSlot, 1, false);
        ...
        if (this.getLiquidAmount() > 0) {
            for (int slot = 2; slot < this.cargoItems.getSlotCount(); slot++) {
                ...
                if (remainingTime <= 0) {
                    ...
                    remainingTime = (int) (BurnUtil.getBurnTime(stack) / gauge.scale()
                            * (Config.ConfigBalance.locoSteamFuelEfficiency / 100.0));
        ...
        if (boilerTemperature > 0) {
            ...
            double radiatedKCalTick =
                    radiatedKCalHr / 60 / 60 / 20 * ConfigBalance.locoHeatTimeScale;
        ...
        if (energyKCalDeltaTick != 0) {
            ...
            // TODO what happens when we change liters per mb
            // FluidQuantity.FromMillibuckets((int) waterLevelMB).Liters()
        ...
        if (throttle != 0 && boilerPressure > 0) {
            ...
            maxPressureTick = maxPressureTick * 0.8; // 20% more pressure gen energyCapability to
                                                     // balance heat loss
        ...
        if (waterUsed != 0) {
            ...
            if (waterUsed > 0 && theTank.getContents() != null) {
                theTank.drain(new FluidStack(theTank.getContents().getFluid(),
                        (int) Math.floor(waterUsed)), false);
        ...
        if (boilerPressure > this.getDefinition().getMaxPSI(gauge) * 1.1
                || (boilerPressure > this.getDefinition().getMaxPSI(gauge) * 0.5
                        && boilerTemperature > 150)) {
            ...
            if (Config.ConfigDamage.explosionsEnabled) {
                this.createExplosion(pos, boilerPressure / 5,
    ...
    @Override
    public void onDragRelease(final Control<?> component) {
    ...
    private double coalEnergyKCalTick() {
        ...
        double coalEnergyKJ = coalEnergyDensity / 9; // Assume each slot is burning 1/9th of a coal
                                                     // block
    ...
    private static class SlotTagMapper implements TagMapper<Map<Integer, Integer>> {
        @Override
        public TagAccessor<Map<Integer, Integer>> apply(final Class<Map<Integer, Integer>> type,
                final String fieldName, final TagField tag) {
            return new TagAccessor<>(
                    (d, o) -> d.setMap(fieldName, o, Objects::toString,
                            i -> new TagCompound().setInteger("val", i)),
    ...
    public boolean cylinderDrainsEnabled() {
        ...
        List<Control<?>> drains = getDefinition().getModel().getControls().stream()
