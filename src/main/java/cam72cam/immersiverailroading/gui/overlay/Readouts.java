...
public enum Readouts {
    LIQUID, SPEED, TEMPERATURE, BOILER_PRESSURE, THROTTLE, REVERSER, TRAIN_BRAKE, TRAIN_BRAKE_LEVER,
    ...
    ENGINE_RPM, CHEST_PRESSURE, HAND_BRAKE;
    ;
    ...
    public float getValue(final EntityRollingStock stock, final float lever) {
        switch (this) {
            case LIQUID:
                return stock instanceof FreightTank
                        ? ((FreightTank) stock).getPercentLiquidFull() / 100f
                        : 0;
            case SPEED:
                double maxSpeed = (stock instanceof Locomotive
                        ? ((Locomotive) stock).getDefinition().getMaxSpeed(stock.gauge).metric()
                        : 0);
                if (maxSpeed == 0) {
                    maxSpeed = 200;
                }
                return (float) Math.abs(
                        ((EntityMoveableRollingStock) stock).getCurrentSpeed().metric() / maxSpeed);
            case TEMPERATURE:
            ...
            case BOILER_PRESSURE:
                return stock instanceof LocomotiveSteam
                        ? ((LocomotiveSteam) stock).getBoilerPressure()
                                / ((LocomotiveSteam) stock).getDefinition().getMaxPSI(stock.gauge)
                        : 0;
            case THROTTLE:
                return stock instanceof Locomotive ? ((Locomotive) stock).getThrottle() : 0;
            case REVERSER:
                return stock instanceof Locomotive ? (((Locomotive) stock).getReverser() + 1) / 2
                        : 0;
            case TRAIN_BRAKE:
            ...
            case BRAKE_PRESSURE:
                return stock instanceof EntityMoveableRollingStock
                        ? ((EntityMoveableRollingStock) stock).getBrakePressure()
                        : 0;
            case COUPLER_FRONT:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).isCouplerEngaged(CouplerType.FRONT)
                                ? 1
                                : 0
                        : 0;
            case COUPLER_REAR:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).isCouplerEngaged(CouplerType.BACK)
                                ? 1
                                : 0
                        : 0;
            case COUPLED_FRONT:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).isCoupled(CouplerType.FRONT)
                                && ((EntityCoupleableRollingStock) stock)
                                        .isCouplerEngaged(CouplerType.FRONT) ? 1 : 0
                        : 0;
            case COUPLED_REAR:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).isCoupled(CouplerType.BACK)
                                && ((EntityCoupleableRollingStock) stock)
                                        .isCouplerEngaged(CouplerType.BACK) ? 1 : 0
                        : 0;
            case COUPLER_SLACK_FRONT:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).slackFrontPercent
                        : 0;
            case COUPLER_SLACK_REAR:
                return stock instanceof EntityCoupleableRollingStock
                        ? ((EntityCoupleableRollingStock) stock).slackRearPercent
                        : 0;
            case BELL:
            ...
            case ENGINE:
                return stock instanceof LocomotiveDiesel
                        ? ((LocomotiveDiesel) stock).isTurnedOn() ? 1 : 0
                        : 0;
            case FRONT_BOGEY_ANGLE:
                return yawToPercent(stock.getDefinition().getModel()
            ...
            case FRONT_LOCOMOTIVE_ANGLE:
                ...
                return front instanceof LocomotiveModel
                        ? yawToPercent(((LocomotiveModel<?, ?>) front)
                                .getFrontLocomotiveYaw((EntityMoveableRollingStock) stock), 90)
                        : 0.5f;
            case REAR_LOCOMOTIVE_ANGLE:
                StockModel<?, ?> rear = stock.getDefinition().getModel();
                return rear instanceof LocomotiveModel
                        ? yawToPercent(((LocomotiveModel<?, ?>) rear)
                                .getRearLocomotiveYaw((EntityMoveableRollingStock) stock), 90)
                        : 0.5f;
            case CYLINDER_DRAIN:
                return stock instanceof LocomotiveSteam
                        && ((LocomotiveSteam) stock).cylinderDrainsEnabled() ? 1 : 0;
            case CARGO_FILL:
                return stock instanceof Freight ? ((Freight) stock).getPercentCargoFull() / 100f
                        : 0;
            case ENGINE_RPM:
                return stock instanceof LocomotiveDiesel
                        ? ((LocomotiveDiesel) stock).getRelativeRPM()
                        : 0;
            case CHEST_PRESSURE:
                return stock instanceof LocomotiveSteam
    ...
    private float yawToPercent(float yaw, final float deltaYaw) {
        yaw = ((yaw % 360) + 360) % 360;
    ...
    public void setValue(final EntityRollingStock stock, final float value) {
        switch (this) {
            case THROTTLE:
            ...
            case TRAIN_BRAKE_LEVER:
                if (stock.getDefinition().isLinearBrakeControl()) {
                    ...
                    if (stock instanceof Locomotive) {
                        ...
                        ((Locomotive) stock).setTrainBrake(Math.max(0, Math.min(1,
                                ((Locomotive) stock).getTrainBrake() + (value - 0.5f) / 80)));
                    }
                }
                break;
            case INDEPENDENT_BRAKE:
            ...
            case COUPLER_FRONT:
                if (stock instanceof EntityCoupleableRollingStock) {
                    ((EntityCoupleableRollingStock) stock).setCouplerEngaged(
                            EntityCoupleableRollingStock.CouplerType.FRONT, value == 0);
                }
                break;
            case COUPLER_REAR:
                if (stock instanceof EntityCoupleableRollingStock) {
                    ((EntityCoupleableRollingStock) stock).setCouplerEngaged(
                            EntityCoupleableRollingStock.CouplerType.BACK, value == 0);
                }
                break;
            case BELL:
                if (stock instanceof Locomotive) {
            ...
            case ENGINE:
                if (stock instanceof LocomotiveDiesel) {
                    ((LocomotiveDiesel) stock)
                            .setTurnedOn(!((LocomotiveDiesel) stock).isTurnedOn());
                }
                break;
            case CYLINDER_DRAIN:
                if (stock instanceof LocomotiveSteam) {
