...
import cam72cam.immersiverailroading.library.PZBMode;
...
public class TileRailBase extends BlockEntityTrackTickable implements IRedstoneProvider {
    ...
    @TagField("pzbMode")
    private PZBMode pzbMode = PZBMode.HZ1000;
    ...
    public void setRailHeight(final float height) {
    ...
    public void setAugment(final Augment augment) {
    ...
    public boolean setAugmentFilter(final String definitionID) {
    ...
    public PlayerMessage nextAugmentRedstoneMode(final boolean isPiston) {
        if (this.augment == null)
            return null;
        switch (this.augment) {
            case DETECTOR:
                detectorMode = StockDetectorMode.values()[((detectorMode.ordinal() + 1)
                        % (StockDetectorMode.values().length))];
                return PlayerMessage.translate(detectorMode.toString());
            case LOCO_CONTROL:
                controlMode = LocoControlMode.values()[((controlMode.ordinal() + 1)
                        % (LocoControlMode.values().length))];
                return PlayerMessage.translate(controlMode.toString());
            case COUPLER:
                if (isPiston) {
                    couplerMode = CouplerAugmentMode.values()[((couplerMode.ordinal() + 1)
                            % (CouplerAugmentMode.values().length))];
            ...
            case FLUID_UNLOADER:
                if (isPiston) {
                    ...
                    return PlayerMessage.translate("immersiverailroading:augment.pushpull."
                            + (this.pushPull ? "enabled" : "disabled"));
                } else {
                    this.redstoneMode = RedstoneMode.values()[(redstoneMode.ordinal() + 1)
                            % RedstoneMode.values().length];
                    return PlayerMessage.translate(redstoneMode.toString());
                }
            case PZB:
                pzbMode = PZBMode.values()[((pzbMode.ordinal() + 1) % (PZBMode.values().length))];
                return PlayerMessage.translate(pzbMode.toString());
            default:
                return null;
        }
    }
    ...
    public void setSnowLayers(final int snowLayers) {
    ...
    private final SingleCache<Vec3i, Vec3i> parentCache =
            new SingleCache<>(parent -> parent.add(getPos()));
    ...
    public void setParent(final Vec3i pos) {
    ...
    @Override
    public void writeUpdate(final TagCompound nbt) {
    ...
    @Override
    public void readUpdate(final TagCompound nbt) {
    ...
    @Override
    public void load(final TagCompound nbt) {
        int version = 0;
    ...
    @Override
    public void save(final TagCompound nbt) {
    ...
    public void setReplaced(final TagCompound replaced) {
    ...
    public TileRailBase getReplacedTile() {
        return replaced != null ? (TileRailBase) getWorld().reconstituteBlockEntity(replaced)
                : null;
    }
     ...
     * net.minecraft.util.math.BlockPos pos, net.minecraft.block.state.IBlockState
     * oldState, net.minecraft.block.state.IBlockState newState) { // This works
     * around a hack where Chunk does a removeTileEntity directly after calling
     * breakBlock // We have already removed the original TE and are replacing it
     * with one which goes with a new block if (this.skipNextRefresh ) { return
     * false; } return super.shouldRefresh(world, pos, oldState, newState); }
     */

    // Called before flex track replacement
    public void setWillBeReplaced(final boolean value) {
    ...
    @Override
    public Vec3d getNextPosition(final Vec3d currentPosition, final Vec3d motion) {
        ...
        if (distanceMetersSq * 0.9 > maxDistance * maxDistance)
            ...
            return MovementTrack.iterativePathing(getWorld(), currentPosition, this,
                    getTrackGauge(), motion, maxDistance);
        return getNextPositionShort(currentPosition, motion);
    }

    private Collection<TileRail> tiles = null;

    public Vec3d getNextPositionShort(final Vec3d currentPosition, final Vec3d motion) {
        if (this.getReplaced() == null) {
            // Simple common case, maybe this does not need to be optimized out of the for
            // loop below?
            ...
            Vec3d potential =
                    MovementTrack.nextPositionDirect(getWorld(), currentPosition, tile, motion);
            if (potential != null)
                return potential;
        ...
        if (tiles == null) {
            ...
            for (TileRailBase current = this; current != null; current =
                    current.getReplacedTile()) {
                TileRail tile =
                        current instanceof TileRail ? (TileRail) current : current.getParentTile();
        ...
        for (TileRail tile : tiles) {
            ...
            Vec3d potential =
                    MovementTrack.nextPositionDirect(getWorld(), currentPosition, tile, motion);
            if (potential != null) {
                // If the track veers onto the curved leg of a switch, try that (with angle
                // limitation)
                // If two overlapped switches are both set, we could have a weird situation, but
                // it's a incredibly unlikely edge case
                if (state == SwitchState.TURN) {
                    // This code is *fundamentally* broken and most of the time no-longer matters
                    // due to the complex parent position logic above
                ...
                // If we are not on a switch curve and closer to our target (or are on the first
                // iteration)
                if (currentPosition == nextPos
                        || !hasSwitchSet && potential.distanceToSquared(predictedPos) < nextPos
                                .distanceToSquared(predictedPos)) {
                    nextPos = potential;
                }
            }
        }
    ...
    public <T extends EntityRollingStock> T getStockNearBy(final Class<T> type) {
        if (overhead == null)
            return null;
    ...
    @Override
    public IInventory getInventory(final Facing side) {
    ...
    @Override
    public ITank getTank(final Facing side) {
        if (this.getAugment() != null) {
            switch (this.getAugment()) {
                case FLUID_LOADER:
                case FLUID_UNLOADER:
    ...
    public void update() {
        ...
        if (ConfigDebug.snowAccumulateRate > 0
                && ((int) (Math.random() * ConfigDebug.snowAccumulateRate * 10) == 0)) {
        ...
        if (ticksExisted > 5 && blockUpdate
                || (ticksExisted % (20 * 5) == 0 && ticksExisted > (20 * 20))) {
            ...
            if (Config.ConfigDamage.requireSolidBlocks && this instanceof TileRail
                    && getWorld().isBlock(getPos(), IRBlocks.BLOCK_RAIL)) {
        ...
        try {
            switch (this.augment) {
                ...
                case WATER_TROUGH:
                    /*
                     * if (this.augmentTank == null) { this.createAugmentTank(); } Tender tender =
                     * this.getStockNearBy(Tender.class, fluid_cap); if (tender != null) {
                     * transferAllFluid(this.augmentTank, tender.getCapability(fluid_cap, null),
                     * waterPressureFromSpeed(tender.getCurrentSpeed().metric())); } else if
                     * (this.ticksExisted % 20 == 0) { balanceTanks(); freight.cargoItems}
                     */
                    break;
                case LOCO_CONTROL: {
                ...
                case DETECTOR: {
                    EntityMoveableRollingStock stock =
                            this.getStockNearBy(EntityMoveableRollingStock.class);
                    ...
                    switch (this.detectorMode) {
                        case SIMPLE:
                            ...
                            break;
                        case SPEED:
                            newRedstone = stock != null
                                    ? (int) Math
                                            .floor(Math.abs(stock.getCurrentSpeed().metric()) / 10)
                                    : 0;
                            break;
                        case PASSENGERS:
                            newRedstone =
                                    stock != null ? Math.min(15, stock.getPassengerCount()) : 0;
                            break;
                        case CARGO:
                            newRedstone = 0;
                ...
                case COUPLER: {
                    EntityCoupleableRollingStock stock =
                            this.getStockNearBy(EntityCoupleableRollingStock.class);
                    if (stock != null) {
                        switch (couplerMode) {
                            case ENGAGED:
    ...
    @Override
    public int getStrongPower(final Facing facing) {
        return getAugment() == Augment.DETECTOR ? this.redstoneLevel : 0;
    }

    @Override
    public int getWeakPower(final Facing facing) {
        return getAugment() == Augment.DETECTOR ? this.redstoneLevel : 0;
    }
    ...
    public SwitchState cycleSwitchForced() {
        ...
        if (tileSwitch != null) {
            newForcedState = SwitchState.values()[(tileSwitch.info.switchForced.ordinal() + 1)
                    % SwitchState.values().length];
    ...
    public void setSwitchForced(final SwitchState newForcedState) {
        TileRail tileSwitch = this.findSwitchParent();
    ...
    /**
     * Finds a parent of <code>this</code> whose type is TrackItems.SWITCH. Returns
     * null if one doesn't exist
     * 
     * @return parent Rail where parent.info.settings.type.equals(TrackItems.SWITCH)
     *         is true, if such a parent exists; null otherwise
     */
    ...
    /**
     * Finds a parent of <code>cur</code> whose type is TrackItems.SWITCH. Returns
     * null if one doesn't exist
     * 
     ...
     *         is true, if such a parent exists; null otherwise
     */
    public TileRail findSwitchParent(final TileRailBase cur) {
        if (cur == null)
            return null;
    ...
    public IBoundingBox getBoundingBox() {
        if (this instanceof TileRailGag
                && (getParent() == null || !getWorld().isBlockLoaded(getParent())))
            // Accessing TEs (parent) in chunks that are currently loading can cause
            // problems
    ...
    @Override
    public boolean onClick(final Player player, final Player.Hand hand, final Facing facing,
            final Vec3d hit) {
        ItemStack stack = player.getHeldItem(hand);
        if (stack.is(IRItems.ITEM_TRACK_EXCHANGER)
                && player.hasPermission(Permissions.EXCHANGE_TRACK)) {
            ...
            if (!track.equals(tileRail.info.settings.track)
                    || !railBed.equals(tileRail.info.settings.railBed)
                    || !gauge.equals(tileRail.info.settings.gauge)) {
                RailInfo info = tileRail.info.withSettings(b -> {
                    b.track = track;
                    b.railBed = railBed;
                    b.gauge = gauge;
                });
                Audio.playSound(getWorld(), getPos(), StandardSound.BLOCK_ANVIL_PLACE,
                        SoundCategory.BLOCKS, 0.3f, 0.2f);
        ...
        if (player.hasPermission(Permissions.AUGMENT_TRACK) && (stack.is(Fuzzy.REDSTONE_TORCH)
                || stack.is(Fuzzy.REDSTONE_DUST) || stack.is(Fuzzy.PISTON))) {
    ...
    @Override
    public boolean tryBreak(final Player player) {
        ...
        try {
            ...
            if (rail.getReplaced() != null) {
                ...
                TileRailGag newGag =
                        (TileRailGag) getWorld().reconstituteBlockEntity(rail.getReplaced());
                if (newGag == null)
                    return true;

                while (true) {
                    if (newGag.getParent() != null
                            && getWorld().hasBlockEntity(newGag.getParent(), TileRail.class)) {
    ...
    public void stockOverhead(final EntityMoveableRollingStock stock) {
        this.overhead = stock;
    }
}
